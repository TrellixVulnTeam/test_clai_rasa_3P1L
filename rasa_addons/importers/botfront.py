...
#from rasa.core.utils import get_file_hash # claichanges
...
class BotfrontFileImporter(TrainingDataImporter):
    ...
    def path_for_nlu_lang(self, lang) -> List[Text]:
        ...
        ospath = os.path.join("nlu", lang)
       # return [x for x in self._nlu_files if f"nlu/{lang}" in x or f"nlu-{lang}" in x] # pemari
        
        return [x for x in self._nlu_files if ospath in x or f"nlu-{lang}" in x]

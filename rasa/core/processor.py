...
ACTION_CLARITY_DATA_NAME = "action_clarity_default_slots"
...
class MessageProcessor:
    ...
    async def _update_tracker_session(
        ...
        if not tracker.applied_events() or self._has_session_expired(tracker):
            ...
            if ACTION_CLARITY_DATA_NAME in self.domain.user_actions_and_forms:
                if 'firstName' not in metadata['clarity']:
                    logger.debug('Could not set the value of slot firstName as it is not passed')
                else:
                    tracker.update(
                        SlotSet("firstname", metadata['clarity']['firstName'])
                    )
                if 'version' not in metadata['clarity']:
                    logger.debug('Could not set the value of slot version as it is not passed')
                else:
                    tracker.update(
                        SlotSet("version", metadata['clarity']['version'])
                    )
                if 'lang' not in metadata['clarity']:
                    logger.debug('Could not set the value of slot language as it is not passed')
                else:
                    tracker.update(
                            SlotSet("language", metadata['clarity']['lang'])
                        )
    ...
    async def log_message(
        ...
        if message.output_channel.name() == 'bot_regression_test_output':
            # clai TEST CHANNEL: send user messages to the output channel
            # if the output channel is the clai output channel

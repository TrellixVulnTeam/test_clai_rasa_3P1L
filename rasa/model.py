...
def get_model_subdirectories(
    ...
    nlu_path = {
        lang: None
        for lang in models_fingerprint.get(FINGERPRINT_CONFIG_NLU_KEY, {})
    }
    
    try:
        for model in nlu_models:
            lang = model.split("-")[1]
            nlu_path['en'] = os.path.join(unpacked_model_path, model)
    except Exception:
        pass
    ...
    if not os.path.isdir(nlu_path['en']): # clai
       nlu_path = None # clai
...
def model_fingerprint(file_importer: "TrainingDataImporter") -> Fingerprint:
    ...
    nlu_config = file_importer.get_nlu_config()
    ...
    return {
        ...
        # FINGERPRINT_CONFIG_NLU_KEY1: _get_fingerprint_of_config(
        ...
        #FINGERPRINT_NLU_LABELS_KEY1: nlu_data.label_fingerprint(),
...
def should_retrain(
    ...
    force_training: bool = False,
    nlu_untrainable: List[Text] = [],  # clai

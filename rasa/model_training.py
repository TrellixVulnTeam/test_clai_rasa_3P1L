...
async def train_async(
    ...
    """
    # file_importer1 = TrainingDataImporter.load_from_config(
...
async def _train_async_internal(
    ...
    new_fingerprint = model.model_fingerprint(file_importer)
    old_model = model.get_latest_model(output_path)
    nlu_untrainable = [l for l, d in nlu_data.items() if d.is_empty()]  # clai
    domain = file_importer.get_domain()  # clai
    core_untrainable = domain.is_empty() or stories.is_empty()  # clai
    ...
    if not force_training:
        fingerprint_comparison = model.should_retrain(
            ...
            old_model,
            train_path,
            # has_e2e_examples=nlu_data.has_e2e_examples(), # clai
            nlu_untrainable=nlu_untrainable,  # clai
        )
    else:
...
async def _do_training(
    ...
    if fingerprint_comparison_result.should_retrain_nlu():
        ...
        _, interpreter_path = model.get_model_subdirectories(model_path)  # clai
    else:
...
async def _train_nlu_with_validated_data(
    ...
    with ExitStack() as stack:
        ...
        config = file_importer.get_nlu_config(retrain_nlu)  # clai
        rasa.shared.utils.cli.print_color(

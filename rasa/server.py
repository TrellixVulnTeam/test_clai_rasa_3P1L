...
def create_app(
    ...
    async def version(request: Request) -> HTTPResponse:
        ...
        return response.json(
            {
                "version": rasa.__version_bf__, # clai
    ...
    async def train(request: Request, temporary_directory: Path) -> HTTPResponse:
        ...
        load_model_after = request.args.get("load_model_after", False) # clai
        ...
        if request.headers.get("Content-type") == YAML_CONTENT_TYPE:
            ...
            load_model_after = request.json.get("load_model_after", load_model_after) # clai
    ...
    async def evaluate_intents(
        ...
        if not cross_validation_folds:
            test_coroutine = _evaluate_model_using_test_set(
                request.args.get("model"), test_data, request.args.get("language"),  # clai
            )
    ...
    async def _evaluate_model_using_test_set(
        model_path: Text, test_data_file: Text, language: Text,  # clai
    ) -> Dict:
        ...
        try:
            ...
            try:
                parsed_data = await app.agent.parse_message_using_nlu_interpreter(
                    data.get("text"), lang=request.json.get("lang"),  # clai
                )
            except Exception as e:
                logger.debug(traceback.format_exc())
                raise ErrorResponse(
...
def _training_payload_from_json(request: Request, temp_dir: Path) -> Dict[Text, Any]:
    ...
    return dict(
        ...
        output=os.environ.get("MODEL_PATH", DEFAULT_MODELS_PATH),  # clai
        ...
        fixed_model_name=request_payload.get("fixed_model_name"),  # clai
        persist_nlu_training_data=True,  # clai
